...
class RetinaFace(object):
	...
	def detect(self, img_list, img_list2, threshold=0.6, keep_first=False):
		...
		if proposals.shape[0]>0:
			...
			if keep_first:
				faces_batch = [[i[0]] for i in faces_batch if i is not None]
				landmarks_batch = [[i[0]] for i in landmarks_batch if i is not None]
		else:
